...
# curs.execute(bank_accounts, [clients[0]])
        out=0
        # test= cursor.execute('mp_output_param_test ? ?','8',out) # don't work
        # test= cursor.execute('mp_output_param_test',['8',out]) # don't work
        # test= cursor.execute('mp_output_param_test ? ?',['8',out]) # don't work
#  curs.execute('call orcl.MYFUNC(TO_DATE(?,?),?,?,?)',
#              (arg_in1, 'yyyy/mm/dd', arg_in2, arg_out1, arg_out2)
#             )
        # test= cursor.execute('DECLARE @count INT; exec [dbo].[mp_output_param_test_1_param]  @col1 = @count OUTPUT; SELECT @count;') # don't work
        test= cursor.execute("DECLARE @count INT;") # work
        test= cursor.execute("DECLARE @count INT;exec mp_output_param_test_1_param @col1 = @count output;") # work
#   https://docs.oracle.com/javase/tutorial/jdbc/basics/storedprocedures.html
#   https://docs.oracle.com/javase/tutorial/jdbc/basics/gettingstarted.html
#         test= cursor.execute("SELECT @count AS 'rowcount';") # work
        # test= cursor.execute('DECLARE @count INT; exec mp_output_param_test_1_param @col1 = @count output; select 1 as one;') # doesn't work
        # test= cursor.execute('DECLARE @count INT; SELECT 1 as one;') # work
        # test= cursor.execute('DECLARE @count INT; exec [dbo].[mp_output_param_test_1_param]  @col1 = @count OUTPUT; SELECT 1 as one;') # don't work
        # test= cursor.execute('mp_output_param_test_1_param @col1=? output;',out) # don't work
        # test= cursor.execute('mp_output_param_test @col1=10 output;',out) # don't work
#         test= cursor.execute('mp_output_param_test ? output',out) # don't work
# DECLARE @count INT;
